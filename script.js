...
if (!firebase.apps.length) {// Firebase SDK চেক
if (typeof firebase === 'undefined') {
    console.error("Firebase SDK লোড হয়নি। index.html-এ Firebase CDN যোগ করুন।");
}
            ...
            fetch('http://localhost:5000/upload-image', {
                ...
                .then(data => {
                    if (data.image_url) {
                        ...
                        console.error('ইমেজ আপলোডে এরর:', data.error);
                    }
                })
                .catch(error => {
                    console.error('ইমেজ আপলোডে এরর:', error);
                });
            clearPreview();
            hideWelcomeMessage();
        }
    ...
    function displayReview(reviewData) {
        ...
        confirmBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                console.error('Confirm বাটনে এরর:', error);
        ...
        if (!isEditable) {
            ...
            reviewContent.querySelectorAll('.review-item').forEach(item => {
                ...
                if (typeof value === 'string' && (value.startsWith('http') || value.startsWith('data:image'))) {
                    ...
                    const replaceIcon = document.createElement('i'); // এখানে ঠিক করা হয়েছে
    ...
    function generatePDF(reviewData, reviewCard) {
        ...
        .then(data => {
            if (data.pdf_url) {
                ...
                throw new Error(data.error || 'পিডিএফ তৈরি ব্যর্থ হয়েছে');
            }
        })
        .catch(error => {
            console.error('পিডিএফ তৈরিতে এরর:', error);
            ...
            if (typeof $ !== 'undefined') {
                $.ajax({
                    ...
                    error: (error) => {
                        ...
                        console.error('Rasa API এরর:', error.status, error.statusText, error.responseText);
                    }
                });
            } else {
                typingDiv.remove();
            ...
            genres.forEach(genre => {
                ...
                genreItem.addEventListener('click', () => {
                    if (genre.message) {
                        ...
                        console.error(`মেসেজ ডিফাইন করা নেই: ${genre.name}`);
    ...
    document.querySelectorAll('.welcome-buttons button[data-genre]').forEach(button => {
        ...
        button.addEventListener('click', () => {
            ...
            if (genre && genre.message) {
                ...
                console.error(`জেনার পাওয়া যায়নি বা মেসেজ ডিফাইন করা নেই: ${genreName}`);
                displayMessage('এই সেবাটি বর্তমানে উপলব্ধ নয়।', 'bot');
                saveChatHistory('এই সেবাটি বর্তমানে উপলব্ধ নয়।', 'bot');
            }
        });
            ...
            try {
                ...
                downloadBtn.addEventListener('click', () => {
                    ...
                    if (pdfUrl) {
                        const link = document.createElement('a');
                        link.href = pdfUrl;
                        const serverFileName = decodeURIComponent(pdfUrl.split('/').pop()); // Cloudinary URL থেকে ফাইলের নাম নিন
                        link.download = serverFileName; // সার্ভারের নাম ব্যবহার করুন, যা .pdf ধারণ করে
                        document.body.appendChild(link);
                        link.click();
                        document.body.removeChild(link);
                    } else {
        ...
        if (!isEditable) {
            ...
            reviewContent.querySelectorAll('.review-item').forEach(item => {
                ...
                if (typeof value === 'string' && (value.startsWith('http') || value.startsWith('data:image'))) {
                    ...
                    const replaceIcon-forwarded = document.createElement('i');
                    replaceIcon.className = 'fas fa-camera replace-image-icon';
                    item.appendChild(replaceIcon);
